...
1. Check that Softeria tools are available in the current task. If connection or account state is unclear, use `mcp__ms365__verify_login`. Do not stop at an unauthenticated result.
   - If no `mcp__ms365__*` tools are exposed, distinguish server startup from Microsoft authentication. On Windows, confirm the Anstar AI marketplace is current and Node.js 20 or later is installed. The bundled launcher resolves standard Node.js locations and does not require `npx` on Codex's inherited `PATH`. Do not edit an installed plugin cache as a repair.
