...
3. Use `bc_actions_search` for the requested entity or capability. Do not enumerate every API or load all schemas into context.
...
5. Use `bc_actions_invoke` only when the described operation is explicitly a read. Never invoke create, update, delete, posting, send, bound actions, ambiguous operations, or bulk mutation tests—even to demonstrate that permission is denied. Unexpected write exposure is a stop condition to report to IT.
